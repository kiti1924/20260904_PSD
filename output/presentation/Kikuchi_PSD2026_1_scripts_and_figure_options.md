...
全17枚を使う場合の推奨順は、**1、2、3、4、5、6、7、8、11、9、10、13、12、15、14、16、17**。

- 2は寓話の状況、3はその結果と開示のジレンマに分ける。
- 11はBaseとSynthesisの区別を説明するページとして、分析結果の前に移す。
- 9は「Key opportunity」の反復を避け、特徴量から分析へ進む方法の説明に使う。
- 13はH1の回帰による照合、15はH2の交互作用の読み方とする。
- 14の限界を結果の後にまとめ、そのまま16の提案につなぐ。

原稿Aをこの順番に並べた読み上げ用ファイルも別途用意する。実際の所要時間は語数に加え、図を指す間やスライド切り替えを含めて見積もる。持ち時間が短い場合は、3を2へ、11を8へ統合し、13・15を質疑用に回す。
